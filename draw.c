...
void createDrawing(){
    int fd = open("drawing",O_CREAT | O_RDWR, 0644);
    char line[256];
    int n;
    for (n = 0; n < 20; n++){
        sprintf(line,"torus\n%lf %lf %lf %lf %lf\n",
        (double)(XRES / 2),(double)(YRES - n * 10),(double)0,(double)n * 2,(double)1);
        write(fd,line,strlen(line));
    }
    sprintf(line,"sphere\n%lf %lf %lf %lf\n",
    (double)(XRES / 2),(double)(YRES /2),(double)0,(double)50);
    write(fd,line,strlen(line));
    sprintf(line,"sphere\n%lf %lf %lf %lf\n",
    (double)((XRES / 2) - 20),(double)(YRES /2 + 20),(double)0,(double)10);
    write(fd,line,strlen(line));
    sprintf(line,"sphere\n%lf %lf %lf %lf\n",
    (double)((XRES / 2) + 20),(double)(YRES /2 + 20),(double)0,(double)10);
    write(fd,line,strlen(line));
    sprintf(line,"sphere\n%lf %lf %lf %lf\n",
    (double)(XRES / 2),(double)(YRES /2 - 20),(double)0,(double)20);
    write(fd,line,strlen(line));
    sprintf(line,"ident\nrotate\n%c %lf\napply\n",'x',(double)30);
    write(fd,line,strlen(line));
...
void add_sphere( struct matrix * edges,
                 ...
                 double r, int step ) {
                     ...
                     int n;
                     for (n = 0; n < sphere->cols; n++){
                         add_edge(edges,sphere->m[0][n],sphere->m[1][n],sphere->m[2][n],
                             sphere->m[0][n],sphere->m[1][n],sphere->m[2][n]);
                     }
                     return;
}
...
struct matrix * generate_sphere(double cx, double cy, double cz,
    double r, int step ) {
        ...
        for (rot = 0; rot < step; rot++){
            re = (double)rot/step;
            for (cir = 0; cir < step; cir++){
                t = (double)cir/step;
                double x = r * cos(M_PI * t) + cx;
                double y = r * sin(M_PI * t) * cos(2 * M_PI * re) + cy;
                double z = r * sin(M_PI * t) * sin(2 * M_PI * re) + cz;
...
void add_torus( struct matrix * edges,
    ...
    double r1, double r2, int step ) {
        ...
        int n;
        for (n = 0; n < torus->cols;n++){
            add_edge(edges,torus->m[0][n],torus->m[1][n],torus->m[2][n],
                torus->m[0][n],torus->m[1][n],torus->m[2][n]);
        }
        return;}
...
struct matrix * generate_torus( double cx, double cy, double cz,
    double r1, double r2, int step ) {
        ...
        for (p = 0; p < step; p++){
            ...
            for (t = 0; t < step; t++){
                cir = (double)t/step;
                double x = cos(rot * 2 * M_PI) * (r1 * cos(cir * 2 * M_PI) + r2) + cx;
                double y = r1 * sin(cir * 2 * M_PI) + cy;
                double z = -sin(rot * 2 * M_PI) * (r1 * cos(cir * 2 * M_PI) + r2) + cz;
